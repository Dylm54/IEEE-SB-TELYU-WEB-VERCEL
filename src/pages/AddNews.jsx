...
function AddNews() {
    ...
    const handlePublish = async (e) => {
        ...
        if (!newImage) {
            return toast.error("Upload the news thumbnail to publish it")
        }
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_API_URL}/news`, formData, {
                headers: {
            ...
            toast.success('Successfully published the news!')
            window.location.href = "#/DashboardNews"
        } catch (error) {
                ...
                <section className='2xl:py-[3.75vw] sm:py-[calc(24px+36*(100vw-576px)/1024)] py-[24px]'>
                    <div className='2xl:max-w-[69.5vw] sm:max-w-[calc(932px+(20px+(7000vw-40320px)/1024)*2)] max-w-[972px] m-auto 2xl:px-[90px] sm:px-[calc(20px+70*(100vw-576px)/1024)] px-[20px]'>
                        ...
                        <div className='flex w-full flex-row gap-2'>
                            ...
                            <input type="text" onChange={e => setAuthor(e.target.value)} className='w-full outline-none lg:!leading-[1.75] 2xl:text-[1.125vw] lg:text-[18px] !leading-[1.5]' />
                        </div>
                        <div className='2xl:h-[2vw] sm:h-[calc(16px+16*(100vw-576px)/1024)] h-[1rem]'></div>
                        <div className='flex w-full flex-row gap-2'>
                            <div className='lg:leading-[1.75] 2xl:text-[1.125vw] lg:text-[18px] leading-[1.5] font-semibold'>Link: </div>
                            <input type="text" onChange={e => setLink(e.target.value)} className='w-full outline-none lg:!leading-[1.75] 2xl:text-[1.125vw] lg:text-[18px] !leading-[1.5]' />
                        </div>
                    </div>
                </section>
            </div>
