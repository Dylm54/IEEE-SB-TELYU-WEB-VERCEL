...
import IEEEwhite from "../../src/assets/IEEE_SB_Tel-U_White.png"
...
export default function Nav() {
  ...
  return (
    <Navbar shouldHideOnScroll maxWidth="full" className={`px-[20px] sm:px-[calc(20px+70*(100vw-576px)/1024)] 2xl:px-[90px] 2xl:h-[100px] md:h-[calc(60px+40*(100vw-576px)/1024)] h-[65px] transition-all fixed ${changeColor ? "" : "changingNav"}`}>
      <NavbarBrand>
        <a href="/">
          <img src={changeColor ? IEEEblack : IEEEwhite} className="h-[37px] lg:h-[45px]" alt="" />
        </a>
      ...
      <NavbarContent justify="end">
        <NavbarItem>
          <div className="right-side hidden sm:flex sm:gap-[20px] lg:gap-[60px]">
            <Dropdown className={`${changeColor ? "white" : "dark"} mt-3`}>
              <DropdownTrigger>
                <h2 className="flex items-center text-xs lg:text-sm gap-2 cursor-pointer font-medium">
                  About Us
                  <MdKeyboardArrowDown fill={changeColor ? "black" : "white"} />
                </h2>
              </DropdownTrigger>
              <DropdownMenu className="w-[340px]" itemClasses={{
                ...
                <DropdownItem href="#/Departments" className="no-underline" startContent={<VscHome size={33} fill="#C0A2FE" />} description="Know more about IEEE SB Telkom University by knowing its organization structure and departments.">
                  <h2 className={`text-xs lg:text-sm text-${changeColor ? "black" : "white"} font-medium no-underline`}>
                    Departements</h2>
                </DropdownItem>
                <DropdownItem href="#/FAQ" className="no-underline" startContent={<FaRegQuestionCircle size={29} fill="#C0A2FE" />} description="Frequently Asked Questions.">
                  <h2 className={`text-xs lg:text-sm text-${changeColor ? "black" : "white"} flex flex-row items-center gap-2 font-medium`}>
                    FAQ</h2>
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
      ...
      <NavbarMenu className={`flex flex-col text-left gap-[50px] ${changeColor ? "text-black" : "dropdownBg"} py-[40px] navHeightVar`}>
